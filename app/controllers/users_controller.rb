class UsersController < ApplicationController
  ...
  def update
    ...
    if @user.identification_image.present?
      # 確認待ち状態へ移行
      @user.update(identification_status: 1)
    end
  ...
  def update_add_info
    if complete_registration_info
      # 正常な処理
      params[:name] = "#{params[:user][:family_name]} #{params[:user][:first_name]}"
      registration_params = params.require(:user).permit(
        :name, :first_name, :first_name_kana, :family_name, :family_name_kana,
        :post_number, :area, :address, :tel, :line_id, :grade,
        :job, :school, :absent_span, :avatar
      )
